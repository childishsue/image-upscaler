...
- **圖片**：提升至 **2K (2560×1440)** 或 **4K (3840×2160)**，支援**批量處理**（最多 20 張）
- **影片**：提升至 **720P / 1080P / 2K / 4K**，逐幀 AI 處理 + 自動複製音訊，支援**批量處理**（最多 10 部）
- **歷史任務**：重開瀏覽器仍可看到最近完成的任務並下載
...
| 圖片 2K / 4K 輸出 | 可選擇 2560×1440 (2K) 或 3840×2160 (4K) 目標解析度 |
| 影片多解析度輸出 | 可選擇 720P / 1080P / 2K / 4K 四種目標解析度 |
| 多格式支援 | 圖片：JPG、PNG、WebP、BMP、TIFF｜影片：MP4、MOV、AVI、MKV、WebM |
| 批量處理 | 圖片一次最多 20 張，依序處理並打包 ZIP |
| 頁籤切換 | 圖片放大與影片放大分頁操作，互不干擾 |
| GPU 加速 | 支援 NVIDIA CUDA GPU 加速（含 RTX 50 系列 Blackwell 架構） |
| 智能模型選擇 | 根據原圖/影片尺寸自動選擇 x2 或 x4 模型 |
| 即時進度 | 圖片/影片即時顯示處理進度（影片顯示幀進度） |
| 前後對比 | 單張圖片結果提供滑桿式對比（支援滑鼠和觸控） |
| 拖曳上傳 | 全程支援拖曳新增（初始上傳區域和選項面板都支援） |
| 自訂檔名 | 下載檔名格式：`原始檔名_解析度_日期_時間.副檔名` |
| 影片音訊保留 | 影片處理時自動偵測並複製原始音訊軌 |
| 歷史任務紀錄 | 重開瀏覽器後仍可看到最近完成的任務並下載（最多 50 筆） |
...
**圖片放大：**
1. 切換到「圖片放大」頁籤
2. **上傳圖片** — 拖曳圖片到虛線框，或點擊選擇檔案（支援多選）
3. **繼續新增** — 已選檔案後，可繼續拖曳到面板新增，或點「繼續新增」按鈕
4. **選擇解析度** — 2K (2560×1440) 或 4K (3840×2160)
5. **開始處理** — 點擊按鈕，觀察即時進度
...
> **注意**：影片處理需逐幀 AI 運算，時間較長。關閉 **CMD 視窗**才會停止伺服器；關閉瀏覽器不影響伺服器運行。
...
範例：
- `風景照_2K_20260215_2218.jpg`
- `TEST_4K_20260215_2220.bmp`
- `家庭影片_1080P_20260215_2230.mp4`
...
│                              #   - 導航列 (顯示 GPU/CPU 狀態)
│                              #   - 頁籤切換 (圖片放大 / 影片放大)
│                              #   - 圖片/影片上傳 (拖曳/點擊/多選，影片批量最多 10 部)
│                              #   - 解析度選擇 (圖片 2K/4K；影片 720P/1080P/2K/4K)
│                              #   - 處理進度 (圖片/影片共用，影片可取消佇列)
│                              #   - 結果頁面 (圖片對比 / 批量 ZIP / 影片個別或 ZIP)
│                              #   - 最近完成的任務 + 「清除所有暫存檔案」按鈕
│                              #   - 伺服器離線偵測與自動重新載入
│                              #   - FFmpeg 未安裝提示
│
...
│                              #   - 頁籤切換 (圖片/影片)，切換後上傳區正確顯示
│                              #   - 圖片：拖曳上傳、多檔管理、輪詢、結果
│                              #   - 影片：批量上傳、佇列取消、輪詢、幀進度、個別/ZIP 下載
│                              #   - 歷史任務載入與渲染、手動清理按鈕
│                              #   - 前後對比滑桿 (滑鼠/觸控)
│                              #   - 伺服器存活偵測，離線提示、恢復時自動 reload
│
├── uploads/                   # [暫存] 使用者上傳的原始圖片/影片
│                              #   檔名: {task_id}.{ext}
│                              #   僅能透過網頁「清除所有暫存檔案」手動清除
│
├── outputs/                   # [暫存] AI 處理後的圖片/影片 + ZIP
│                              #   圖片: {task_id}_upscaled.{ext}
│                              #   影片: {task_id}_upscaled.mp4
│                              #   ZIP: {batch_id}_all.zip
│                              #   僅能透過網頁「清除所有暫存檔案」手動清除
│
